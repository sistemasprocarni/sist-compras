  ...
  try {
    ...
    if (uploadType === 'supplier') {
      ...
      for (let i = 0; i < jsonData.length; i++) {
        ...
        if (!supplierCode) {
          failureCount++;
          errors.push({ row: rowNum, data: rowData, reason: 'Código del proveedor faltante.' }); // Updated error message
          continue;
        }
        if (!materialCode) {
          failureCount++;
        ...
        // Find supplier_id by code
        const { data: supplierLookup, error: supplierLookupError } = await supabaseClient
          .from('suppliers')
          .select('id')
          .eq('code', supplierCode) // Changed from rif to code
          .single();

        if (supplierLookupError || !supplierLookup) {
          console.warn(`[bulk-upload] Supplier with code '${supplierCode}' not found for row ${rowNum}. Skipping relation.`); // Updated log
          failureCount++;
          errors.push({ row: rowNum, data: rowData, reason: `Proveedor con código '${supplierCode}' no encontrado.` }); // Updated error message
          continue;
        }
        ...
        if (existingRelation) {
          ...
          console.log(`[bulk-upload] Updated supplier_material relation for supplier ${supplierCode}, material ${materialCode}`); // Updated log
        } else {
          // Insert new relation
          dbOperation = await supabaseClient
            .from('supplier_materials')
          ...
          console.log(`[bulk-upload] Inserted new supplier_material relation for supplier ${supplierCode}, material ${materialCode}`); // Updated log
        }
