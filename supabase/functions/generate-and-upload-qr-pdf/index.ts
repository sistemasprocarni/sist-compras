...
serve(async (req) => {
  ...
  try {
    ...
    if (!authHeader) {
      console.warn('[generate-and-upload-qr-pdf] Unauthorized: No Authorization header');
    ...
    if (!user) {
      console.warn('[generate-and-upload-qr-pdf] Unauthorized: Invalid user session');
      return new Response('Unauthorized', { status: 401, headers: corsHeaders });
    }

    const { requestId } = await req.json();
    console.log(`[generate-and-upload-qr-pdf] Generating PDF for quote request ID: ${requestId} by user: ${user.email}`);
    ...
    if (requestError || !request) {
      console.error(`[generate-and-upload-qr-pdf] Error fetching quote request ${requestId}:`, requestError);
    ...
    if (itemsError) {
      console.error(`[generate-and-upload-qr-pdf] Error fetching quote request items for ${requestId}:`, itemsError);
    ...
    if (request.companies?.logo_url) {
      try {
        ...
        console.warn(`[generate-and-upload-qr-pdf] Could not load company logo:`, logoError);
      }
    }
    ...
    const fileName = `temp/${requestId}_${Date.now()}.pdf`;
    ...
    if (uploadError) {
      console.error('[generate-and-upload-qr-pdf] Error uploading PDF:', uploadError);
    ...
    if (signedUrlError) {
      console.error('[generate-and-upload-qr-pdf] Error creating signed URL:', signedUrlError);
    ...
    console.log(`[generate-and-upload-qr-pdf] PDF uploaded and signed URL created for user: ${user.email}`);
    ...
    console.error('[generate-and-upload-qr-pdf] Error:', error);
