...
serve(async (req) => {
  ...
  try {
    ...
    console.log(`[generate-po-pdf-cloudinary] Generating PDF for order ID: ${orderId} by user: ${user.email}`);
    ...
    if (orderError || !order) {
      console.error(`[generate-po-pdf-cloudinary] Error fetching order ${orderId}:`, orderError);
    ...
    if (itemsError) {
      console.error(`[generate-po-pdf-cloudinary] Error fetching order items for ${orderId}:`, itemsError);
    ...
    if (order.companies?.logo_url) {
      try {
        ...
        console.warn(`[generate-po-pdf-cloudinary] Could not load company logo:`, logoError);
      }
    }
    ...
    if (!Deno.env.get('CLOUDINARY_CLOUD_NAME') || !Deno.env.get('CLOUDINARY_API_KEY') || !Deno.env.get('CLOUDINARY_API_SECRET')) {
      console.error('[generate-po-pdf-cloudinary] Cloudinary credentials are not set.');
    ...
    const fileName = `orden_compra_${order.sequence_number}.pdf`;
    const uploadResult = await new Promise((resolve, reject) => {
      const uploadStream = cloudinary.uploader.upload_stream(
        {
          folder: 'procarni_documents',
          resource_type: 'raw',
          public_id: fileName,
          overwrite: true,
        },
    ...
    console.log(`[generate-po-pdf-cloudinary] PDF uploaded to Cloudinary: ${publicUrl}`);
    ...
    console.error('[generate-po-pdf-cloudinary] General Error:', errorMessage, error);
